...
const char* ssid = "Maria"; // Cambia por tu SSID
const char* password = "44843368"; // Cambia por tu contraseña
const char* mqtt_server = "broker.emqx.io"; // Cambia por tu broker si es necesario
...
void procesar_comando_mqtt(const String& cmd) {
  ...
  if (cmd == "1") {
    Serial.println("cambia manual");
    current_state = MANUAL_ON;
    encender_rele();
    log_estado("Ventilador ENCENDIDO (MQTT)");
  } else if (cmd == "2") {
    Serial.println("cambia modo auto");
    current_state = MANUAL_OFF;
    apagar_rele();
